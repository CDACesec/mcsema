...
llvm::Function *GetOrCreateMcSemaInitializer(void) {
  ...
  if (!gInitFunc) {
    ...
    ir.CreateRetVoid();
    LOG(INFO)<<"IR is :"<<remill::LLVMThingToString(entry);
    LOG(INFO)<<"already done is :"<<remill::LLVMThingToString(already_done);
    LOG(INFO)<<"lazy xref is :"<<remill::LLVMThingToString(lazy_xref);
  }
  return gInitFunc;
}
