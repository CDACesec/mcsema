...
#include <llvm/Support/Debug.h> // added
...
static bool TryLiftTerminator(TranslationContext &ctx,
                              llvm::BasicBlock *block,
                              remill::Instruction &inst, bool is_last) {
  switch (inst.category) {
    ...
    case remill::Instruction::kCategoryConditionalCall:                         
    {
      // inst like `bgezal` can be a branch or a function
      // call, so conditional call can generate the
      // ir for bgezal, depending on if it is a function call case of
      // bgezal or branching case.
      bool is_function_call = false;
      for(auto func : ctx.cfg_module->ea_to_func){
        if (func.first == inst.branch_taken_pc)
        {
...
static bool LiftInstIntoBlock(TranslationContext &ctx,
                              ...
                              bool is_last) {
  remill::Instruction inst;
  remill::Instruction delayed_insn;

  auto inst_addr = ctx.cfg_inst->ea;
  auto &bytes = ctx.cfg_inst->bytes;

  inst.has_ref_info = ctx.cfg_inst->has_ref_info;                 
  ...
  ctx.lifter->LiftIntoBlock(inst, block, false);
  ...
  // we assume brach cases of bgezal do not have delay slot
  // so we don't delay the IR generation part here.
  // in case of call instruction, we will lift generate the llvm `call`
  // instruction after lifting the instruction in the delay slot.
  if ( (inst.category == remill::Instruction::kCategoryConditionalCall) &&
     (!is_function_call) ) {
    TryLiftTerminator(ctx, block, inst, is_last);
  }
  
  // Handle delay slot based on the CFG provided by IDA-Pro
  // Assume that for instructions like bgezal, branch case
  // doesn't have delay slot, but call case does.
  if (ctx.cfg_inst->is_delayed) {

    ctx.cfg_inst = ctx.delayed_inst;
    gArch->DecodeInstruction(ctx.cfg_inst->ea, ctx.cfg_inst->bytes, inst);
...
static void LiftBlockIntoFunction(TranslationContext &ctx) {
  ...
  // save the delay slot instruction, and lift it after the
  // next instruction
  for (auto cfg_inst : ctx.cfg_block->instructions) {
    auto is_last = (++i) >= num_insts;

    if(cfg_inst->has_delay_slot) {
      ctx.delayed_inst = cfg_inst;
    }
  ...
  LOG(INFO)
      << "successor_eas are: ";
  for (auto succ_ea : follows) {
    LOG(INFO) << "0x" << std::hex << succ_ea;
  }
