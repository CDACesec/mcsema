...
remill::LiftStatus InstructionLifter::LiftIntoBlock(
  ...
  if (is_delayed) {
    mem_ref = GetAddress(ctx.delayed_inst->mem);
    imm_ref = GetMaskedAddress(ctx.delayed_inst->imm);
    disp_ref = GetMaskedAddress(ctx.delayed_inst->disp);
  }
  mem_ref_used = false;
  disp_ref_used = false;
  imm_ref_used = false;

  auto status = this->remill::InstructionLifter::LiftIntoBlock(inst, block, is_delayed);
...
llvm::Value *InstructionLifter::LiftAddressOperand(
  ...
  if ((mem.base_reg.name.empty() && mem.index_reg.name.empty()) ||
     (((mem.base_reg.name == "PC") || (mem.base_reg.name[0] == 'W'))
      &&  mem.index_reg.name.empty())) { 
    if (mem_ref) {
      mem_ref_used = true;
      return mem_ref;
