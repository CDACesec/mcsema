...
static const char *ThreadPointerName(void) {
  ...
  switch (gArch->arch_name) {
    ...
    case remill::kArchMips32:
     tp_name = "TLS_BASE"; /* for now, until we come up with a better name */
     break;
...
struct CallingConventionInfo {
 ...
 private:
  ...
  void CreateArgumentsConstraintTables(void) {
    ...
    } else if (gArch->IsMips()) {
        std::vector<ArgConstraint> mips32_args = {
          // {"A0", kIntegralLeast32},
          // {"A1", kIntegralLeast32},
          // {"A2", kIntegralLeast32},
          // {"A3", kIntegralLeast32},
  ...
  void CreateReturnConstraintTable(void) {
    ...
    if (gArch->IsAMD64()) {
      ...
      ConstraintTable mips32_table = {
        {"W2", kIntegralLeast32},
        {"W3", kIntegralLeast32},
      };
      ret_tables.emplace_back(llvm::CallingConv::C,
                              std::move(mips32_table));
    }
  }
...
void CallingConvention::StoreArguments(
  ...
  CHECK(gArch->IsX86() || gArch->IsAMD64() || gArch->IsAArch64() || gArch->IsMips());
...
void CallingConvention::AllocateReturnAddress(llvm::BasicBlock *block) {
  ...
  // The stack grows down on x86/amd64.
    ...
    //return address is passed through register $ra or W31
    return;
...
void CallingConvention::FreeReturnAddress(llvm::BasicBlock *block) {
  ...
  // The stack grows down on x86/amd64.
    ...
    auto W31 = remill::FindVarInFunction(block, "W31"); //$ra
    llvm::IRBuilder<> ir(block);
    auto ret_addr = ir.CreateLoad(W31);
    remill::StoreProgramCounter(block, ret_addr);
...
llvm::Value *GetTLSBaseAddress(llvm::IRBuilder<> &ir) {
  ...
  // 32-bit x86.
    ...
    // which llvm function would give tls base for mips?
    LOG(ERROR)
        << "Assuming the `thread.pointer` intrinsic gets us the base "
        << "of thread-local storage for MIPS32.";
    auto func = llvm::Intrinsic::getDeclaration(
        gModule, llvm::Intrinsic::thread_pointer); /* find a mips specific version for this */
    return ir.CreatePtrToInt(ir.CreateCall(func), gWordType);
  }
