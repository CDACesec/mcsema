...
NativeModule *ReadProtoBuf(const std::string &file_name,
  ...
  for (const auto &cfg_segment : cfg.segments()) {
    ...
    LOG(INFO)
        << "Found segment at " << segment->name << " [" << std::hex << segment->ea
  ...
  for (const auto &cfg_func : cfg.funcs()) {
    ...
    LOG(INFO)
        << "Found function at " << func->name << " at " << std::hex
        << func->ea << std::dec;
    ...
    if (func->is_exported) {
      ...
      LOG(INFO)
          ...
          << func->ea << std::dec << "  implemented by " << func->lifted_name;
  ...
  for (const auto &cfg_func : cfg.funcs()) {
    ...
    for (const auto &cfg_block : cfg_func.blocks()) {
      ...
      for (const auto &cfg_inst : cfg_block.instructions()) {
        ...
        inst->has_ref_info = cfg_inst.has_ref_info();
        inst->has_delay_slot = cfg_inst.has_delay_slot();
        inst->is_delayed = cfg_inst.is_delayed();
